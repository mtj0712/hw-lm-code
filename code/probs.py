...
class CountBasedLanguageModel(LanguageModel):
    ...
    def sample(self, max_length: int=50) -> str:
        words = [BOS, BOS]

        for _ in range(max_length):
            p = []
            for v in self.vocab:
                p.append(self.prob(words[-2], words[-1], v))
            next_word_i = torch.multinomial(torch.tensor(p, dtype=torch.float), 1)[0].item()
            next_word = self.vocab[next_word_i]
            if next_word == EOS:
                break
...
class BackoffAddLambdaLanguageModel(AddLambdaLanguageModel):
    ...
    def prob(self, x: Wordtype, y: Wordtype, z: Wordtype) -> float:
        ...
        p_unigram = ((self.event_count[(z,)] + self.lambda_) /
                (self.context_count[()] + self.lambda_ * self.vocab_size))

        p_bigram = ((self.event_count[y, z] + self.lambda_ * self.vocab_size * p_unigram) /
                (self.context_count[(y,)] + self.lambda_ * self.vocab_size))

        return ((self.event_count[x, y, z] + self.lambda_ * self.vocab_size * p_bigram) /
                (self.context_count[x, y] + self.lambda_ * self.vocab_size))
...
class EmbeddingLogLinearLanguageModel(LanguageModel, nn.Module):
    ...
    def __init__(self, vocab: Vocab, lexicon_file: Path, l2: float, epochs: int) -> None:
        ...
        with lexicon_file.open(mode="r") as f:
            lexicon_size, dim = f.readline().split()
            lexicon_size, dim = int(lexicon_size), int(dim)

            lexicon_tokens = [None] * lexicon_size
            lexicon_embeddings = [None] * lexicon_size
            for i in range(lexicon_size):
                token, *embedding = f.readline().split()
                if token == "OOL":
                    OOL_i = i                   # OOL_i stores the index of OOL in the lexicon
                lexicon_tokens[i] = token
                lexicon_embeddings[i] = [float(entry) for entry in embedding]

        self.embeddings = [None] * len(vocab)
        for i, token in enumerate(vocab):
            try:
                j = lexicon_tokens.index(token)
            except ValueError:
                j = OOL_i

            self.embeddings[i] = lexicon_embeddings[j]
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        p_tilde = logits[self.find_index_in_vocab(z)]
        normalizer = torch.logsumexp(logits, 0)
        return p_tilde - normalizer
